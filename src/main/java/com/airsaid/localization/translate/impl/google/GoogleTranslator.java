...
public class GoogleTranslator extends AbstractTranslator {
    ...
    private static String TRANSL_URL = "https://clients5.google.com/translate_a/t?";
...
//               .url("https://clients5.google.com/translate_a/t?client=dict-chrome-ex&sl=auto&tl=zh-cn&q=dog")
        ...
        try {
            ...
            List<String> list = GsonUtil.getInstance().getGson().fromJson(resultText, new TypeToken<List<String>>() {
            }.getType());

            StringBuffer stringBuffer = new StringBuffer();
            if (list != null && list.size() > 0) {
                for (String iteam : list) {
                    stringBuffer.append(iteam);
                }
            }
    ...
    private String getRequestParams2(Lang fromLang, Lang toLang, String text) {

        StringBuffer params2 = new StringBuffer();
        params2.append("client=dict-chrome-ex&");
        params2.append("sl=" + fromLang.getCode() + "&");
        params2.append("tl=" + toLang.getCode() + "&");
        params2.append("q=" + text);
